...
def parse_user_groups(input_file):
    user_groups = []
    ...
    with open(input_file, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        for line in lines:
            ...
            # Start of user group configuration block (first level)
            if current_indentation == 0 and stripped_line == "config user group":
                group_config_started = True
                indentation_level = current_indentation
                continue

            # End of user group configuration block (first level)
            ...
            if current_group and current_indentation == indentation_level + 8:
                if stripped_line.startswith("set member "):
